...
bool UserManager::loadUsersFromDb()
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName))
    {
        ...
        printf("UserManager::LoadUsersFromDb failed, please check params: dbserver: %s, dbusername: %s, , dbpassword: %s, dbname: %s",
               m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }

    std::unique_ptr<QueryResult> res_ptr(pConn->query("SELECT f_user_id, f_username, f_password FROM t_user"));
    if (nullptr == res_ptr)
    {
        // LOGI("UserManager::_Query error, dbname: %s", m_strDbName.c_str());
        printf("UserManager::_Query error, dbname: %s", m_strDbName.c_str());
        return false;
    }
    ...
    for (auto &Row : Rows)
    {
        ...
        u.userid = convertFromString<int>(strValues[0]);
        u.username = strValues[1];
        u.password = strValues[2];
        m_allCachedUsers.push_back(u);
        printf("userid: %d, username: %s, password: %s\n", u.userid, u.username.c_str(), u.password.c_str());
    }
    return true;
}
